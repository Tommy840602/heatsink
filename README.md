...
Phase 3.31 adds a fail-closed, read-only AWS audit around that bootstrap. Before execution it accepts only a fully created, available `CREATE` change set whose parameters, original template SHA-256, capability, resource inventory, and Add-only actions exactly match the reviewed source. After an authorized deployment it reads back stack termination protection, direct S3/KMS/OIDC/IAM controls, exact trust and inline-policy statements, and uses IAM simulation to prove required state/lock operations remain allowed while representative state deletion and infrastructure mutations remain denied. A static AST contract permits only approved describe/get/list/simulate AWS calls; no create, update, delete, execute, or drift-detection operation exists. Fixture tests run in CI without AWS credentials, and no live audit or deployment is claimed.

Phase 3.32 adds an AWS-independent production foundation for RKE2, Rook-managed Ceph, OpenBao, Keycloak, and Harbor. Official component versions and chart digests are locked; RKE2 uses HA embedded etcd, CIS checks, encrypted Secrets, audit logging, and compressed snapshots. Ceph is fail-closed with no automatic disk selection, encrypted explicit OSDs, retained replicated RBD, and retained TLS-only RGW pools. OpenBao uses TLS-enabled three-node Raft with retained audit/data volumes and separate CSI roles for Thanos Receive, Store, and Compactor; its privileged node CSI provider is isolated as an external-mode release in `kube-system` instead of weakening the OpenBao server namespace. Keycloak requires external PostgreSQL and TLS; Harbor requires external PostgreSQL/Valkey, internal TLS, two or more replicas, and Ceph S3. CI renders every pinned chart plus the 27-resource self-hosted Thanos overlay and rejects unsafe disk discovery, embedded credentials, non-retained data, shared object-store authority, or single-node production settings. No server or Kubernetes cluster was modified.
