...
1. Validate `infra/aws-bootstrap/production-terraform-plan.yml` with pinned `cfn-lint` and `scripts/validate_aws_bootstrap.py`. Create and fully review a CloudFormation change set through a separately authenticated bootstrap identity; execution is a distinct authorized operator action. Never manage the state foundation from the state it stores.
...
3. Protect the GitHub `production-plan` Environment with required reviewers, self-review prevention where available, and deployment restricted to protected `main`. Its OIDC role trust must use audience `sts.amazonaws.com`, the exact live Environment subject, and `StringEquals` only.
4. Configure only Environment variables from reviewed bootstrap outputs; store no access key. The plan role may Get/Put the exact state object, Get/Put/Delete only its exact `.tflock`, and read managed resources. It must not delete state or mutate IAM, KMS, or the Thanos bucket. Versioning is the recovery control for authorized or compromised state overwrite.
5. Run Terraform 1.15.8 formatting/validation and both Terraform contract validators locally or in CI. These checks use no AWS credentials.
6. Dispatch `Production Terraform plan` from `main`, enter that exact 40-character commit SHA, select `PLAN_ONLY`, and approve the Environment deployment. Any ref, SHA, account, region, role, bucket, prefix, or OIDC mismatch fails before AWS authentication.
7. Review the value-free job summary. Delete, replacement, dependency-lock drift, and more than 50 changed resources are stop conditions. The workflow has no apply step, uploads no saved plan, and removes its temporary binary plan, JSON, and log even after failure.
8. Confirm all public-access blocks, BucketOwnerEnforced ownership, versioning, KMS default encryption, bucket keys, TLS deny, `prevent_destroy`, and `force_destroy = false` remain in the plan.
9. Confirm current objects have no S3 expiration. Receive has list/get/put without delete, Store has list/get only, and Compactor alone has list/get/put/delete; every IRSA trust uses exact `aud` and `sub` equality.
10. Treat this plan as review evidence only. Applying it requires a future separately reviewed role, workflow, approval policy, and explicit change authorization. After an authorized apply exists, feed outputs into the S3 and EKS renderers; never copy credentials, state, or plan files into `.runtime` or Git.
